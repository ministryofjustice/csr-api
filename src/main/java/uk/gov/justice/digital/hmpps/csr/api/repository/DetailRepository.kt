...
class DetailRepository(val jdbcTemplate: NamedParameterJdbcTemplate) {
    ...
    fun getOvertimeDetails(from: LocalDate, to: LocalDate, quantumId: String): Collection<Detail> {
    ...
    fun getShiftDetails(from: LocalDate, to: LocalDate, quantumId: String): Collection<Detail> {
        return jdbcTemplate.query(
                GET_SHIFT_DETAILS,
                MapSqlParameterSource()
                        .addValue("from", from)
    ...
    companion object {
        ...
        // If this is actually this simple we can change the above to in (4000,1000)
        val GET_SHIFT_DETAILS = """
        SELECT DISTINCT 
          CASE
            WHEN TW_SCHEDULE.TASK_START < 0 THEN TW_SCHEDULE.ON_DATE-1
            ELSE TW_SCHEDULE.ON_DATE
        ...
        AND   TW_SCHEDULE.ON_DATE BETWEEN :from AND :to
        AND   TW_SCHEDULE.LAYER = -1        -- TOP LAYER
        AND   TW_SCHEDULE.LEVEL_ID = 1000   -- Time Recording line only
        AND   LOWER(SM_USER.NAME) = LOWER(:quantum_id)
